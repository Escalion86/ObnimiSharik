const versionHistory = [
  {
    ver: '0.14',
    date: '2021-12-21T00:15:00+07:00',
    add: [
      'Функция мультивыбора для карточек (пока только функция удаления)',
      'Теперь районы доставки влияют на главную страницу сайта (отображаются на ней показывая стоимость). Настройка самой выделенной територии производится через аккаунт разработчика',
      'В формы товаров, наборов, типов товаров и типов наборов добавлен статус "Показывать на сайте". Отключенный пункт означает, что карточка не будет показана на главной странице сайта (ни в поиске - нигде)',
    ],
    fix: ['Ошибка формы типа набора', 'Правки стилей сайта (главной страницы)'],
  },
  {
    ver: '0.13',
    date: '2021-11-26T23:50:00+07:00',
    add: [
      'Теперь для сотрудников можно задать субдолжность, для того чтобы их можно было выбирать ответственными в заказах',
    ],
    fix: ['Ошибка возникающая, если не выставлять статус в форме заказа'],
  },
  {
    ver: '0.12',
    date: '2021-11-25T06:00:00+07:00',
    add: ['В заказ теперь можно вносить примеры работ (фотографии)'],
    update: [
      'Форма заказа',
      'При создании заказа автоматически проставляется город доставки Красноярск',
      'В форме заказа при добавлении новой строки в списке товаров, наборов и расходов товаров - автоматически раскрывается список товаров (раньше нужно было дополнительно кликать в новую строку). Также фильтр "Показывать только в наличии" (>0), теперь включен по умолчанию',
    ],
    fix: [
      'Ошибка сохранения расходных товаров в заказе (после сохранения изменений заказа, происходило дублирование)',
      'Отображение месяца в карточках и формах',
      'Расчет скидки в заказе при изменении процента',
      'Ошибка при создании новых типов товаров или наборов',
      'Ошибка отсутствия плавающей кнопки создания новой карточки, если на странице нет карточек',
    ],
  },
  {
    ver: '0.11',
    date: '2021-11-23T15:05:00+07:00',
    add: [
      'Поиск (фильтр) "по имени" для товаров, наборов, типов, клиентов и сотрудников',
    ],
    update: [
      'При создании транзакции, теперь после выбора заказа, автоматически будет подставляться и клиент, который оформлял выбранный заказ',
    ],
    fix: ['Ошибка позваляющая сохранять карточку транзакции без указания типа'],
  },
  {
    ver: '0.10',
    date: '2021-11-23T06:35:00+07:00',
    add: [
      'Сортировка "по цене" для движений товаров',
      'Фильтр по стоимости в движении товаров',
      'Теперь на карточках заказов можно кликать на товары и наборы для открытия всплывающего меню с набором функций',
      'Плавающая кнопка добавления карточек справа внизу (старая кнопка добавления удалена)',
    ],
    fix: [
    ...
    update: ['Улучшение адаптивности заголовка'],
  },
  {
    ver: '0.09a',
    emergency: true,
